...
public class SolicitanteController {
    ...
    public ResponseEntity<SolicitanteResponse> cadastrar(@RequestBody SolicitanteRequest request) {
        SolicitanteResponse response = solicitanteService.cadastrar(request);
        return ResponseEntity.ok(response);
    }

    @PutMapping("/{id}/atualizar")
    public SolicitanteResponse atualizar(@PathVariable Integer id, @RequestBody SolicitanteRequest request) {
